...
We noticed that our image was 489MB. Even though we used OpenJDK:8-JRE as the base image, but it still contains a lot of JDK functionality that is not needed by our application. JDK 9 introduced module systems that that allows you to selectively include the functionality needed for your application, and leaving everything else behind. We'll look at how we our image size can be reduced using the tools provided by JDK 9 onwards.
